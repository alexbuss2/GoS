...
@router.post("/mobile/google", response_model=MobileTokenExchangeResponse)
async def exchange_google_id_token(
    payload: MobileTokenExchangeRequest,
    db: AsyncSession = Depends(get_db),
):
    """Exchange Google ID token for app token (mobile flow)."""
    logger.info("[mobile/google] Received Google ID token exchange request")

    try:
        id_claims = await validate_id_token(payload.id_token)
    except IDTokenValidationError as e:
        logger.warning("[mobile/google] ID token validation failed: %s", e.message)
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=e.message)

    email = id_claims.get("email", "")
    name = id_claims.get("name") or derive_name_from_email(email)
    platform_sub = id_claims.get("sub")
    if not platform_sub:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid ID token")

    auth_service = AuthService(db)
    user = await auth_service.get_or_create_user(platform_sub=platform_sub, email=email, name=name)
    ...
    return MobileTokenExchangeResponse(token=app_token, user=user_response)
